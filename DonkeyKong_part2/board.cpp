...
void Board::print(bool with_color, int sleep_duration) const {
	system("cls");
	char ch;
	// for color mode - print the characters according to their color
	if (with_color) {
		for (int i = 0; i < GameConfig::MAX_HEIGHT; i++) {
			Sleep(5);
...
// // this function prints the legend game
	...
	while (screen1.get(c) && curr_row < GameConfig::MAX_HEIGHT) {
		...
		if (curr_col < GameConfig::MAX_WIDTH) {
			...
			case (char)characters::ghost_l:
				super_ghosts_positions.push_back({ curr_col, curr_row });
				originalBoard[curr_row][curr_col++] = SPACE;
				break;
