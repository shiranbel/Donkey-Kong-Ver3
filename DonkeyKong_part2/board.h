...
class Board {
	...
	Position start_pos_hammer;                                 // for saving start pos of hammer
	std::vector<Position> ghosts_positions;                    // for handle regular ghosts positions from map
	std::vector<Position> super_ghosts_positions;			   // for handle super ghosts positions from map
...
public:
	...
	// ctor
	Board(bool with_color, const std::string& filename, bool is_silent) {
	...
	void print(bool with_color, int sleep_duration) const;					// prints the board on screen
	...
	void resetNumOfSuperGhosts() { index_super_ghosts = 0; }                // reset index of super ghosts vector
	...
	bool getIsSilent() const { return isSilent; }									// get's if the level is in silent mode
	...
	Position getStartPosSuperGhost()  { return super_ghosts_positions[index_super_ghosts++]; }	    // return start game position of super ghost by index	
	std::vector<Position> getVectorGhost() const { return ghosts_positions; }	    // return ghosts positions vector
	std::vector<Position> getVectorSuperGhost() const { return super_ghosts_positions; }	// return super ghosts positions vector
