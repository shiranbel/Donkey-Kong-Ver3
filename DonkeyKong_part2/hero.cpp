...
void Hero::erase() {
	...
	if (pBoard->getSetColor()) {										// if color mode
		...
		case (char)Board::Floor_chars::ladder:							// for ladder
			...
			break;
		default:														// for others
			pBoard->setColor((int)Board::colors::yellow);
			break;
		}
	...
	for (size_t i = 0; i < numKeys; i++) {
		...
		if (std::tolower(key) == (char)keys::up) {										// handle up movement
			if (curr_pos_ch != (char)Board::Floor_chars::ladder && !isOnFloor(curr_pos_ch)) {			// jump - if my position is not ladder or floor	
				if (!isOnFloor(curr_pos_ch_under)) { return false; }                           // checks if is there a floor under mario - to avoid double jump
			...
			if (!isOnFloor(curr_pos_ch_under) && curr_pos_ch != (char)Board::Floor_chars::ladder) { return false; }	// block going down while jumping
			else {
				dir = { 0, 1 };															// change direction to straight down
				pressedDown = true;			
				return true;
				// remember that it's down from the player and not free fall
			}
		}
		else if (std::tolower(key) == (char)keys::stay) {								// stay 
			if (!isOnFloor(curr_pos_ch_under) && curr_pos_ch != (char)Board::Floor_chars::ladder) { return false; }	// block stay while jumping
			else {
				dir = { 0, 0 };	
				return true;// change direction to stay
			}
		}
		else if (std::tolower(key) == keys[i]) { // right, left
			dir = directions[i];
			return true;
		}
