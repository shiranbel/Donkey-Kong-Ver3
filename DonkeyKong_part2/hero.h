...
class Hero {
	...
	void draw(char c) const {
		if (!(pBoard->getIsSilent())) {	// draws a given char at the hero's position
...
public:
	...
	bool keyPressed(char key);										// gets the key that was pressed by the user
	...
	bool isCollision() const {return (pBoard->getCharFromCurr(pos.getX(), pos.getY()) == (char)Board::characters::barrel || pBoard->getCharFromCurr(pos.getX(), pos.getY()) == (char)Board::characters::ghost_s || pBoard->getCharFromCurr(pos.getX(), pos.getY()) == (char)Board::characters::ghost_l);} // checks if the hero's position is on barrel
	void changelives() {		// decreases the remaining lives by 1
		lives--;
